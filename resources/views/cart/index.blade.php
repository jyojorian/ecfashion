...
<div class="container">
    ...
    <button onClick="location.href='{{route('product.index')}}'" class="cart__purchase btn btn-primary">
        商品メイン画面に戻る
    </button>
    ...
    <!-- <script>
        function MoveCheck() {
            ...
            if (res == true) {
                // OKなら移動
                window.location.href = '{{route('cart.redirect')}}';
            } else {
                // キャンセルならアラートボックスを表示
                alert("移動をやめまーす。");
            }
    ...
    <button onClick="location.href='{{route('cart.redirect')}}'" class="cart__purchase btn btn-primary">
        購入画面に移動する
    </button>
    @else
